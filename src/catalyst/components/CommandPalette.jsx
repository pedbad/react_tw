...
function CommandPalette({ items = [] }) {
  return (
    <div className="card p-5">
      <div className="surface-panel flex items-center gap-3 px-4 py-2">
      ...
      <ul className="mt-4 space-y-2">
        {items.map((item) => (
          <li
            key={item.id}
            className="surface-panel flex items-center justify-between px-4 py-2 text-sm text-text-primary transition-base hover:border-primary/40 hover:bg-primary/5"
          >
            {item.label}
